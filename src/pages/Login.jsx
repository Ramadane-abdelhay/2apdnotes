...
import { AlertCircle, Lock, Loader2, User, GraduationCap } from 'lucide-react';

export default function Login() {
  const [email, setEmail] = useState(''); // This acts as the 'Username'
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('student_username');
    if (savedUser) {
      setEmail(savedUser);
      setRememberMe(true);
    }
  ...
  const handleStudentLogin = async (e) => {
    ...
    try {
      ...
      if (!isStudent) {
        ...
        throw new Error("Accès refusé : Compte étudiant requis.");
      }

      // Handle "Remember Me"
      if (rememberMe) {
        localStorage.setItem('student_username', email);
      } else {
      ...
      <div className="w-full max-w-[400px]">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-slate-100 p-8 md:p-10">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-indigo-600 text-[11px] font-bold uppercase tracking-[0.2em] mt-1.5">Espace Étudiant</p>
          </div>
          ...
          <form onSubmit={handleStudentLogin} className="space-y-4">
            <div className="space-y-4">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-12 pr-4 py-4 text-[14px] font-medium outline-none transition-all focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 text-slate-700" 
                />
              </div>
              ...
              <label className="flex items-center cursor-pointer group">
                <div className="relative">
                  <input 
                    ...
                    className="sr-only" 
                  />
                  <div className={`w-5 h-5 border-2 rounded-lg transition-all flex items-center justify-center ${rememberMe ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 bg-white group-hover:border-indigo-400'}`}>
            ...
            <button 
              type="submit" 
              disabled={loading} 
              className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold text-[14px] mt-2 shadow-[0_10px_20px_-5px_rgba(0,0,0,0.1)] hover:bg-indigo-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
